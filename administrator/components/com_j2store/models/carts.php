...
//echo '<pre>'; print_r($app->input->get('front')); exit;
		if(!isset($product_id)) {
			$errors['error'] =  array('general'=>JText::_('J2STORE_PRODUCT_NOT_FOUND'));
			return $errors;
		}
		$session['front'] = $app->input->get('front');
...
$app = JFactory::getApplication();
		...
		if (!empty($cart))
		{
			...
			//$keynames ['front'] = $app->input->get('front');
			...
			$item->front_image = $app->input->getString('front');
			$item->back_image = $app->input->getString('back');
			//echo '<pre>'; print_r($app->input->getString('back')); exit;
