...
class J2StoreModelProducts extends F0FModel {
    ...
    public function _buildQueryJoins($query) {
        ...
        $query->select('#__j2store_variants.lumiseid');
    ...
    function _buildWhereQuery(&$query){
        ...
        if($state->search){
            ...
            $query->where('('.
                ...
                $db->qn('#__j2store_variants').'.'.$db->qn('lumiseid').' LIKE '.$db->q('%'.$state->search.'%').'OR '.
    ...
    public function getSearchProduct(){
        ...
        if($search){
            $query->where(
                ...
                $db->qn('#__j2store_variants').'.'.$db->qn('lumiseid').' LIKE '.$db->q('%'.$search.'%').' OR '.
    ...
    function _sfBuildWhereQuery(&$query){
        ...
        if($state->search){
            $query->where(
                ...
                $db->qn('#__j2store_variants').'.'.$db->qn('lumiseid').' LIKE '.$db->q('%'.$state->search.'%').'AND  a.state =1 OR '.
